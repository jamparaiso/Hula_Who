...
local function nextScene4Event ( event )
...
function scene: createScene (event)
		...
		local scene4Image = display.newImageRect (  "images/final4.png", 300, 400 );
		scene4Image.x = _W*0.5; scene4Image.y =210
		
screenGroup:insert(background)
screenGroup:insert(scene4Image) 
end


function scene: enterScene(event)

local nextScene4Button = widget.newButton
 
	{
			...
			id = "Next_Scene4_Button",
			onEvent = nextScene4Event,
	} 
...
local skipButton = widget.newButton
{
	...
	onEvent = skipToPlayerMenu,
			...
			button:insert(nextScene4Button)
