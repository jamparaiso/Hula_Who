...
local function nextScene5Event ( event )
...
function scene: createScene (event)
		...
		local scene5Image = display.newImageRect (  "images/final5.png", 300, 400 );
		scene5Image.x = _W*0.5; scene5Image.y =210
		
screenGroup:insert(background)
screenGroup:insert(scene5Image) 
end


function scene: enterScene(event)

local nextScene5Button = widget.newButton
 
	{
			...
			id = "Next_Scene5_Button",
			onEvent = nextScene5Event,
	} 
...
local skipButton = widget.newButton
{
	...
	onEvent = skipToPlayerMenu,
			...
			button:insert(nextScene5Button)
